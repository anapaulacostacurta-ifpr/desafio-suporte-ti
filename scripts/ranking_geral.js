  ...
  fetch(csvPath)
    ...
    .then(data => {
      ...
      const ranking = lines.map(line => {
        ...
        const total_calculado = (parseFloat(nome) || 0) + (parseFloat(email_padrao) || 0) + (parseFloat(foto) || 0) + (parseFloat(avatar) || 0);
        console.log(total)
      ...
      row2.innerHTML = `<td colspan="6">Total Geral</td><td>${total_geral}</td>`;
